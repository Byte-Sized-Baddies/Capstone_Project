...
type HiveStatsViewProps = {
  onBack: () => void;
};

export default function HiveStatsView({ onBack }: HiveStatsViewProps) {
  ...
  const countToday = completedTasks.filter((t) =>
    t.completedAt?.startsWith(todayKey)
  ).length;
  const countMonth = completedTasks.filter(
    (t) => t.completedAt && new Date(t.completedAt).getMonth() === currentMonth
  ).length;
  ...
  const progressPercent = Math.min(
    100,
    Math.round((countToday / dailyGoal) * 100)
  );
  ...
  const handleDecreaseGoal = () => {
    if (dailyGoal > 1) setDailyGoal(dailyGoal - 1);
  };

  const handleIncreaseGoal = () => {
    setDailyGoal(Math.min(50, dailyGoal + 1));
  };
          ...
          <View style={styles.hexagonWrapper}>
            ...
            <View style={styles.hexTextOverlay}>
              ...
              <Text style={styles.tasksCompletedText}>
                {countToday} / {dailyGoal} Completed
              </Text>
            </View>
          </View>
        </View>
                    ...
                    <View
                      style={[
                        styles.bar,
                        {
                          ...
                          backgroundColor: dayData.height >= 100 ? "#FFA000" : "#FFC107",
                        },
                      ]}
                    />
                  </View>
