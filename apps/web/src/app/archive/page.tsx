...
export default function ArchivePage() {
  ...
  const loadArchivedTasks = async () => {
    ...
    const { data: catRows } = await supabase
      .from("categories_v2").select("id, name").eq("user_id", user.id);
  ...
  useEffect(() => {
    if (authReady) loadArchivedTasks();
  }, [authReady]);
  ...
  const restoreSelected = async () => {
    ...
    const ids = [...selected];
    await supabase.from("tasks_v2").update({ is_archived: false }).in("id", ids).eq("user_id", user.id);
  ...
  const deleteSelected = async () => {
    ...
    if (!user) { setActionLoading(false); return; }
    const ids = [...selected];
    await supabase.from("tasks_v2").delete().in("id", ids).eq("user_id", user.id);
    setTasks(prev => prev.filter(t => !selected.has(t.id)));
    setSelected(new Set());
    setActionLoading(false);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: t.bg }}>
        <div className="text-center"><div className="text-5xl mb-4">🐝</div>
          <div className="text-sm font-medium" style={{ color: t.textDim }}>Loading your archive...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 max-w-4xl mx-auto slide-up">
        ...
        <div className="mb-6 p-4 rounded-2xl flex items-center gap-3"
          ...
          <div>
            ...
            <div className="text-xs" style={{ color: t.textDim }}>
              Archived tasks are hidden from your dashboard. Restore them anytime or delete them permanently.
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl flex-1 min-w-48"
            ...
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M20 20l-3-3" stroke={t.textDim} strokeWidth="2" strokeLinecap="round" />
            </svg>
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search archived tasks..." className="bg-transparent outline-none text-sm flex-1"
              style={{ color: t.text }} />
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={restoreSelected} disabled={actionLoading}
                className="px-3 py-2 rounded-xl text-xs font-semibold"
                style={{ background: t.success + "20", color: t.success }}>
                {actionLoading ? "…" : "↩ Restore"}
              </button>
              <button onClick={deleteSelected} disabled={actionLoading}
                className="px-3 py-2 rounded-xl text-xs font-semibold"
          ...
          {tasks.length > 0 && selected.size === 0 && (
            <button onClick={clearAllArchived} disabled={actionLoading}
        ...
        {filtered.length > 0 && (
          <div className="flex items-center gap-3 mb-3 px-1">
            <button onClick={selectAll} className="flex items-center gap-2 text-xs font-medium"
        ...
        {loading ? (
          <div className="py-20 text-center">
            ...
            <p className="text-sm" style={{ color: t.textDim }}>Loading archive...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-20 text-center rounded-2xl" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            ...
            <p className="text-sm" style={{ color: t.textDim }}>
              {search ? "Try a different search term" : "Tasks you archive will appear here"}
            </p>
            <button onClick={() => router.push("/dashboard")} className="mt-6 px-6 py-3 rounded-xl text-sm font-bold"
              style={{ background: t.accent, color: t.accentText }}>
              Go to Dashboard
            </button>
          </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          {task.due !== "No date" && (
                            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: t.surfaceHover, color: t.textDim }}>📅 {task.due}</span>
                          )}
                          {task.done && (
                            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: t.success + "20", color: t.success }}>✓ Completed</span>
                          )}
                          {task.archivedAt && (
