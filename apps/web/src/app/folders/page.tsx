...
export default function FoldersPage() {
  ...
  const loadFolders = async () => {
    ...
    // Collaborators for owned folders
  ...
  const createFolder = async () => {
    ...
    const { data, error } = await supabase
      .from("folders")
      .insert({ name, color: newFolderColor, user_id: user.id })
    ...
    setFolders(prev => [{
      id: data.id, name: data.name, color: data.color ?? newFolderColor,
      owner: user.id, isOwner: true, taskCount: 0, collaborators: [],
    }, ...prev]);
    setNewFolderName(""); setNewFolderColor(FOLDER_COLORS[0]);
    setShowCreateModal(false); setActionLoading(false);
  };
  ...
  const leaveFolder = async (folder: Folder) => {
    if (!confirm(`Leave "${folder.name}"? You'll lose access.`)) return;
    setActionLoading(true);
    const { data: userData } = await supabase.auth.getUser();
    const user = userData?.user;
    if (!user) { setActionLoading(false); return; }
  ...
  const addCollaborator = async () => {
    ...
    const { error } = await supabase.from("folder_members").insert({
      folder_id: activeFolder.id, user_id: invitedUser.id, role: "editor"
    });
    ...
    if (error) { alert(`Failed to share: ${error.message}`); }
      ...
      const updated = {
        ...activeFolder,
        collaborators: [...activeFolder.collaborators, { user_id: invitedUser.id, email: invitedUser.email, role: "editor" }]
      };
  ...
  const removeCollaborator = async (collabUserId: string, collabEmail: string) => {
    ...
    const { error } = await supabase.from("folder_members")
      .delete().eq("folder_id", activeFolder.id).eq("user_id", collabUserId);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: t.bg }}>
        <div className="text-center"><div className="text-5xl mb-4">🐝</div>
          <div className="text-sm font-medium" style={{ color: t.textDim }}>Loading folders...</div>
        </div>
      </div>
    );
  }
    ...
    <main style={{ minHeight: "100vh", background: t.bg, color: t.text, transition: "background 0.3s ease" }}>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-72 flex flex-col transition-transform duration-300"
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleTheme} className="w-8 h-8 rounded-lg flex items-center justify-center"
                style={{ background: t.surfaceHover }}>{isDark ? "☀️" : "🌙"}</button>
          ...
          <div className="flex items-center gap-3 mb-8 p-3 rounded-2xl" style={{ background: t.surfaceHover }}>
            {avatarDataUrl
              ...
              : <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm"
                  style={{ background: t.accent, color: t.accentText }}>{getInitials()}</div>}
        ...
        <div className="p-6" style={{ borderTop: `1px solid ${t.border}` }}>
          <button onClick={handleLogout} className="w-full py-2.5 rounded-xl text-sm font-medium"
      ...
      <header className="sticky top-0 z-30 px-6 py-4 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setSidebarOpen(true)} className="w-10 h-10 rounded-xl flex items-center justify-center"
            style={{ background: t.surfaceHover, color: t.textMuted }}>☰</button>
          <div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={toggleTheme} className="w-9 h-9 rounded-xl flex items-center justify-center"
            style={{ background: t.surfaceHover, border: `1px solid ${t.border}` }}>{isDark ? "☀️" : "🌙"}</button>
      ...
      <div className="p-6 max-w-6xl mx-auto slide-up">
        ...
        {loading ? (
          <div className="py-24 text-center">
            ...
            <p className="text-sm" style={{ color: t.textDim }}>Loading folders...</p>
          </div>
        ) : folders.length === 0 ? (
          /* Empty state */
          <div className="py-24 text-center rounded-3xl" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            ...
            <button onClick={() => setShowCreateModal(true)}
              ...
              style={{ background: t.accent, color: t.accentText }}>+ Create First Folder</button>
          </div>
        ) : (
          <div className="space-y-8">
            ...
            {ownedFolders.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold uppercase tracking-wider" style={{ color: t.textDim }}>
                    My Folders ({ownedFolders.length})
                  </h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {/* Create new tile */}
                  <button onClick={() => setShowCreateModal(true)}
                    className="folder-tile flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border-2 border-dashed min-h-[180px] transition-all"
            ...
            {sharedFolders.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold uppercase tracking-wider" style={{ color: t.textDim }}>
                    Shared With Me ({sharedFolders.length})
                  </h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-3xl p-6 modal-in" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={newFolderName} onChange={e => setNewFolderName(e.target.value)}
                  ...
                  placeholder="e.g. Work Projects" autoFocus
              ...
              <div className="p-4 rounded-xl flex items-center gap-3" style={{ background: t.surfaceHover }}>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
                  style={{ background: newFolderColor + "30", border: `2px solid ${newFolderColor}` }}>📁</div>
                <div>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowCreateModal(false)} className="flex-1 py-3 rounded-xl text-sm font-medium"
                style={{ background: t.surfaceHover, color: t.textMuted }}>Cancel</button>
              <button onClick={createFolder} disabled={actionLoading} className="flex-1 py-3 rounded-xl text-sm font-bold"
                style={{ background: t.accent, color: t.accentText }}>
      ...
      {showEditModal && activeFolder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-3xl p-6 modal-in" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={editName} onChange={e => setEditName(e.target.value)}
                  ...
                  autoFocus className="w-full px-4 py-3 rounded-xl text-sm" style={inputStyle} />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider block mb-3" style={{ color: t.textDim }}>Color</label>
                <div className="flex gap-2 flex-wrap">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowEditModal(false)} className="flex-1 py-3 rounded-xl text-sm font-medium"
                style={{ background: t.surfaceHover, color: t.textMuted }}>Cancel</button>
              <button onClick={saveEdit} disabled={actionLoading} className="flex-1 py-3 rounded-xl text-sm font-bold"
                style={{ background: t.accent, color: t.accentText }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-3xl p-6 modal-in" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowShareModal(false)} className="w-8 h-8 rounded-lg flex items-center justify-center"
                style={{ background: t.surfaceHover, color: t.textMuted }}>✕</button>
            </div>
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={addCollaborator} disabled={shareLoading}
            ...
            {/* Collaborator list */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider block mb-3" style={{ color: t.textDim }}>
                People with access ({activeFolder.collaborators.length})
              </label>
              {activeFolder.collaborators.length === 0 ? (
                <div className="py-6 text-center rounded-xl" style={{ background: t.surfaceHover }}>
                  <p className="text-sm" style={{ color: t.textDim }}>Only you have access</p>
                </div>
              ) : (
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {activeFolder.collaborators.map(collab => (
                    <div key={collab.user_id} className="flex items-center justify-between p-3 rounded-xl"
                      ...
                      <button
                        onClick={() => removeCollaborator(collab.user_id, collab.email)}
                        disabled={removeLoading === collab.user_id}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium flex-shrink-0 ml-2"
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
              style={{ background: folder.color + "25" }}>
              {folder.isOwner ? "📁" : "🤝"}
            </div>
            <div className="min-w-0">
          ...
          <div className="relative flex-shrink-0">
            <button onClick={e => { e.stopPropagation(); setMenuOpen(v => !v); }}
              ...
              style={{ background: menuOpen ? t.surfaceHover : "transparent", color: t.textMuted }}>
              ⋯
            </button>
            {menuOpen && (
