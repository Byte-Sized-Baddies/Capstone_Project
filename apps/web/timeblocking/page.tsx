...
const priorityColor = (p: Priority, dark: boolean) => dark ? ({
  High: "#ef4444", Medium: "#FFC107", Low: "#22c55e",
}[p]) : ({ High: "#dc2626", Medium: "#d97706", Low: "#16a34a" }[p]);

const BLOCK_COLORS = [
  "#FFC107","#22c55e","#3b82f6","#a855f7","#ef4444","#f97316","#ec4899","#14b8a6",
];
...
export default function TimeBlockingPage() {
  ...
  // Load blocks from localStorage keyed by date
  useEffect(() => {
  ...
  const handleScheduleDrop = (e: React.DragEvent) => {
    ...
    if (draggingBlock) {
      ...
      const updated = blocks.map(b =>
        b.id === draggingBlock.id ? { ...b, startHour: h, startMin: m } : b
      );
      saveBlocks(updated);
      setDraggingBlock(null);
    } else if (draggingTask) {
      // New block — show duration picker
      setPendingTask(draggingTask);
      setPendingHour(h);
      setPendingMin(m);
      setDurationHrs(1);
      setDurationMins(0);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: t.bg }}>
        <div className="text-center"><div className="text-5xl mb-4">🐝</div>
          <div className="text-sm font-medium" style={{ color: t.textDim }}>Loading your schedule...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <main style={{ minHeight: "100vh", background: t.bg, color: t.text, transition: "background 0.3s ease" }}>
      ...
      <div className="flex h-[calc(100vh-73px)]">
        ...
        {/* ── TASK PANEL (left) ───────────────────────────────────────────── */}
        <div className="w-72 flex-shrink-0 flex flex-col overflow-hidden slide-up"
            ...
            {tasks.length === 0 ? (
              ...
              <>
                ...
                {unscheduledTasks.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold px-1 mb-2" style={{ color: t.textDim }}>
                    ...
                    {unscheduledTasks.map(task => {
                      ...
                      return (
                        <div key={task.id}
                          className="task-chip p-3 rounded-xl mb-2"
                          draggable
                ...
                {tasks.filter(tk => scheduledTaskIds.has(tk.id)).length > 0 && (
                  <div>
                    <div className="text-xs font-semibold px-1 mb-2 mt-4" style={{ color: t.textDim }}>
                      SCHEDULED ({tasks.filter(tk => scheduledTaskIds.has(tk.id)).length})
                    </div>
                    {tasks.filter(tk => scheduledTaskIds.has(tk.id)).map(task => {
                      const pc = priorityColor(task.priority, isDark);
                      return (
                        <div key={task.id}
                          className="task-chip p-3 rounded-xl mb-2 opacity-60"
                          draggable
                          onDragStart={e => handleTaskDragStart(e, task)}
                          style={{ background: t.surfaceHover, border: `1px solid ${t.border}`, borderLeft: `3px solid ${pc}` }}>
                          <div className="flex items-start gap-2">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-3 flex items-center justify-between flex-shrink-0"
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => goDay(-1)} className="w-8 h-8 rounded-lg flex items-center justify-center"
            ...
            <div className="flex items-center gap-2">
              ...
              {blocks.length > 0 && (
                <button onClick={() => { if (confirm("Clear all blocks for this day?")) saveBlocks([]); }}
                  ...
                  style={{ background: t.danger + "20", color: t.danger }}>Clear day</button>
              )}
            </div>
          </div>

          {/* Hour grid */}
          <div ref={scheduleRef} className="flex-1 overflow-y-auto relative"
            ...
            {isToday && (
              <div className="absolute left-0 right-0 z-10 pointer-events-none flex items-center"
                style={{ top: currentMinTotal / 60 * HOUR_HEIGHT }}>
            ...
            {/* Time blocks (absolute positioned) */}
            {blocks.map(block => {
              ...
              return (
                <div key={block.id}
                  className="block-item absolute rounded-xl overflow-hidden z-10 group"
                  draggable
                  ...
                  <div className="p-2 h-full flex flex-col justify-between" style={{ background: "rgba(0,0,0,0.15)" }}>
                    <div>
                      ...
                      {heightPx > 44 && (
                        <div className="text-xs text-white opacity-80 mt-0.5">{formatDuration(block.durationMins)}</div>
                      )}
                    </div>
                    {heightPx > 56 && task.category && (
                      <div className="text-xs text-white opacity-70 truncate">{task.category}</div>
                    )}
                  </div>
                  {/* Remove button */}
                  <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-3xl p-6 modal-in" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
            ...
            <p className="text-sm mb-5" style={{ color: t.textDim }}>
              How long will <strong style={{ color: t.text }}>{pendingTask.text}</strong> take?
            </p>
            ...
            <div className="p-3 rounded-xl mb-4 flex items-center gap-2"
              ...
              <span className="text-sm font-semibold" style={{ color: t.text }}>
                Starting at {formatHour(pendingHour)}{pendingMin > 0 ? `:${String(pendingMin).padStart(2, "0")}` : ""}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setDurationHrs(Math.max(0, durationHrs - 1))}
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-lg font-bold"
                    style={{ background: t.surfaceHover, color: t.text }}>−</button>
                  <div className="flex-1 text-center text-xl font-bold" style={{ color: t.text }}>{durationHrs}</div>
                  <button onClick={() => setDurationHrs(Math.min(12, durationHrs + 1))}
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-lg font-bold"
                    style={{ background: t.surfaceHover, color: t.text }}>+</button>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setDurationMins(durationMins === 0 ? 45 : durationMins - 15)}
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-lg font-bold"
                    style={{ background: t.surfaceHover, color: t.text }}>−</button>
                  <div className="flex-1 text-center text-xl font-bold" style={{ color: t.text }}>{String(durationMins).padStart(2, "0")}</div>
                  <button onClick={() => setDurationMins(durationMins === 45 ? 0 : durationMins + 15)}
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-lg font-bold"
                    style={{ background: t.surfaceHover, color: t.text }}>+</button>
                </div>
              </div>
            ...
            {(durationHrs > 0 || durationMins > 0) && (
              <div className="text-center text-sm font-semibold mb-4" style={{ color: t.accent }}>
                {formatDuration(durationHrs * 60 + durationMins)} block
              </div>
            )}
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2 flex-wrap">
                {BLOCK_COLORS.map(color => (
                  <button key={color} onClick={() => setBlockColor(color)}
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDurationModal(false)} className="flex-1 py-3 rounded-xl text-sm font-medium"
                style={{ background: t.surfaceHover, color: t.textMuted }}>Cancel</button>
              <button onClick={confirmBlock}
                disabled={durationHrs === 0 && durationMins === 0}
